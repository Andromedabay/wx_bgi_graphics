...
namespace bgi
{
    ...
    struct BgiState
    {
        ...
        // --- User input hook callbacks (optional; fire after library logic) ---
        WxbgiKeyHook         userKeyHook{nullptr};         ///< Called after keyCallback logic; may be NULL.
        WxbgiCharHook        userCharHook{nullptr};        ///< Called after charCallback logic; may be NULL.
        WxbgiCursorPosHook   userCursorPosHook{nullptr};   ///< Called after cursorPosCallback logic; may be NULL.
        WxbgiMouseButtonHook userMouseButtonHook{nullptr}; ///< Called after mouseButtonCallback logic; may be NULL.
        WxbgiScrollHook      userScrollHook{nullptr};      ///< Called after scrollCallback logic; may be NULL.

        // --- Scroll state ---
        double scrollDeltaX{0.0}; ///< Accumulated horizontal scroll; cleared by wxbgi_get_scroll_delta().
        double scrollDeltaY{0.0}; ///< Accumulated vertical scroll; cleared by wxbgi_get_scroll_delta().

        // --- Input default-behavior flags ---
        // Bitmask of WXBGI_DEFAULT_* constants. Controls which library default
        // behaviors (key queuing, cursor tracking, pick on click, scroll accumulation)
        // are active. Set via wxbgi_set_input_defaults(); initial value = WXBGI_DEFAULT_ALL.
