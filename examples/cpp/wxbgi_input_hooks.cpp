 ...
 *
 *  - **Key hook**         — status panel shows last key + action;
...
static void BGI_CALL hookKey(int key, int /*scancode*/, int action, int mods)
{
    ...
    if      (key == WXBGI_KEY_LEFT)  gMarkerX -= kStep;
    else if (key == WXBGI_KEY_RIGHT) gMarkerX += kStep;
    else if (key == WXBGI_KEY_UP)    gMarkerY -= kStep;
    else if (key == WXBGI_KEY_DOWN)  gMarkerY += kStep;
    // 'C': cycle stamp colour
    else if (key == 67 && action == WXBGI_KEY_PRESS)
    {
...
// ---------------------------------------------------------------------------
// Per-frame draw callback (called by wx timer)
// ---------------------------------------------------------------------------
...
{
    wxbgi_wx_app_create();
    wxbgi_wx_frame_create(kW, kH, "wx_BGI input hooks demo — Esc to exit");
