...
show_usage() {
    cat <<'EOF'
Usage: run_openlb_pipe_3d_wx_slider_demo.sh [options]

Build and run the wx_bgi_graphics 3D OpenLB pipe demo with the wxWidgets flow
slider panel on Debian, Ubuntu, or Debian/Ubuntu-based WSL2.
...
Environment overrides:
  ...
  BUILD_DIR    CMake build directory (default: /tmp/wx_bgi_graphics-openlb-wx-slider-build).
EOF
}

log() {
    printf '[openlb-wx-slider-demo] %s\n' "$*"
}

fail() {
    printf '[openlb-wx-slider-demo] ERROR: %s\n' "$*" >&2
    exit 1
}
...
main() {
    ...
    local build_dir="${BUILD_DIR:-/tmp/wx_bgi_graphics-openlb-wx-slider-build}"
