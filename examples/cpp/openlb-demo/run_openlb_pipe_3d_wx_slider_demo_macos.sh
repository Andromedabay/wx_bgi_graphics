...
show_usage() {
    cat <<'EOF'
Usage: run_openlb_pipe_3d_wx_slider_demo_macos.sh [options]

Build and run the wx_bgi_graphics 3D OpenLB pipe demo with the wxWidgets flow
slider panel on macOS. Assumes Homebrew is already installed.

Options:
  --test                  Build and run the demo's short validation mode.
  --wireframe             Start the 3D panels in wireframe mode.
  --flat                  Start the 3D panels in flat-shaded mode.
  --smooth                Start the 3D panels in smooth-shaded mode (default).
  ...
  --flow-velocity-ms N    Set the initial characteristic flow velocity in m/s.
...
Environment overrides:
  ...
  BUILD_DIR    CMake build directory (default: /tmp/wx_bgi_graphics-openlb-wx-slider-build-macos).
EOF
}

log() {
    printf '[openlb-wx-slider-demo-macos] %s\n' "$*"
}

fail() {
    printf '[openlb-wx-slider-demo-macos] ERROR: %s\n' "$*" >&2
    exit 1
}
...
main() {
    ...
    local build_dir="${BUILD_DIR:-/tmp/wx_bgi_graphics-openlb-wx-slider-build-macos}"
    ...
    log "building wxbgi_openlb_pipe_3d_wx_slider_demo"
    cmake --build "${build_dir}" -j --target wxbgi_openlb_pipe_3d_wx_slider_demo

    local demo_bin="${build_dir}/wxbgi_openlb_pipe_3d_wx_slider_demo"
    ...
    if [[ "${render_mode_set}" -eq 0 ]]; then
        demo_args=(--smooth "${demo_args[@]}")
    fi
